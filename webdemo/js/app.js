...
async function addSeongdongMask(map, L) {
  const res = await fetch("layers/seongdong_boundary.geojson");
...
export async function init() {
  ...
  async function requestRoute() {
    if (!start || !end) return;
    // Live routing runs on the SafeStep backend (FastAPI + Valhalla), which is not
    // part of this static demo. Show a notice instead of calling /api/route.
    msg(
      "ℹ Live routing runs on the SafeStep backend (FastAPI + Valhalla) — not in this " +
        "static demo. Explore the Seongdong-gu highlight, modes and accessibility layers; " +
        "full code is on GitHub."
    );
  }

  async function doSearch(q, resultsEl, kind) {
    // Address search uses Kakao geocoding on the backend — disabled in this static demo.
    clearEl(resultsEl);
    resultsEl.innerHTML =
      `<div class="addrItem">Address search uses Kakao geocoding on the backend — ` +
      `disabled in this static demo. Pan/zoom the map to explore.</div>`;
  }
